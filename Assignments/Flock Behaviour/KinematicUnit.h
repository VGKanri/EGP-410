...
public:
	...
	KinematicUnit(const Vector2D& position, float orientation, const Vector2D& velocity, float rotationVel, float maxVelocity = 0.0f, float maxAcceleration = 0.0f);
	~KinematicUnit();
	...
	bool getFlee() const { return mIsFlee; };
	void setFlee(bool flee) { mIsFlee = flee; };
