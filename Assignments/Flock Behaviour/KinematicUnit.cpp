...
KinematicUnit::KinematicUnit(const Vector2D &position, float orientation, const Vector2D &velocity, float rotationVel, float maxVelocity, float maxAcceleration)
	:Kinematic(position, orientation, velocity, rotationVel)
	, mpCurrentSteering(NULL)
	, mMaxVelocity(maxVelocity)
	, mMaxAcceleration(maxAcceleration)
...
{
	WanderAndSeek* pWanderAndSeekSteering = new WanderAndSeek(this, gpGame->getPlayerUnit(), true);
	setSteering(pWanderAndSeekSteering);
}

void KinematicUnit::setPosition(Vector2D pos)
{
