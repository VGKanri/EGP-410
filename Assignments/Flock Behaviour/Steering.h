...
public:
	...
	Steering( const Vector2D& theLinear = gZeroVector2D, float theAngular = 0.0f, bool applyDirectly = false ):mLinear(theLinear),mAngular(theAngular),mIsWander(false) {};
	Steering( const Steering& rhs ):mLinear(rhs.mLinear), mAngular(rhs.mAngular), mApplyDirectly(rhs.mApplyDirectly),mIsWander(false){};
	~Steering(){};
	...
	bool getIsWander() const { return mIsWander; };
	void setIsWander(bool wander) { mIsWander = wander; };
